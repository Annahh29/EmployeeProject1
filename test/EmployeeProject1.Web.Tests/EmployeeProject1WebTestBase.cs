...
{
    public abstract class EmployeeProject1WebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static EmployeeProject1WebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(EmployeeProject1WebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<EmployeeProject1DbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<EmployeeProject1DbContext, Task> action)
        {
            using (var context = IocManager.Resolve<EmployeeProject1DbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<EmployeeProject1DbContext, Task<T>> func)
        {
            T result;
